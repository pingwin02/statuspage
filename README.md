...
- **Convenient Check Grouping:** Autocomplete and dropdown selection of existing groups in the admin panel, with automatic group inheritance when creating new checks.
...
- **Cache-First & Progressive Live Checks:** Cached statuses are served immediately when cache is valid; when checks run (after cache expiration or returning to dashboard after configuration changes), pills start in loading state, the refresh button is locked, and services resolve progressively in real time without queuing or repeated runs.
- **Immediate Refresh Cancellation on Admin Navigation:** Entering the admin panel or saving configuration immediately aborts any in-flight background checks, preventing background resource consumption and ensuring configuration changes refresh cleanly once upon returning to the dashboard.
