...
- **Cache-First & Progressive Live Checks:** Cached statuses are served immediately when cache is valid; when checks run (after cache expiration or returning to dashboard after configuration changes), manual checks display immediately, live network checks start in loading state, the refresh button is locked, and services resolve progressively in real time without queuing or repeated runs.
